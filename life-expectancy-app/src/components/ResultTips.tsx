...
export default function ResultTips({ data }: { data: FormData }) {
  const [idx, setIdx] = useState(0);
  const sec = sections[idx];
  const entries = Object.entries(data).filter(([k]) =>
    sec.keys ? sec.keys.includes(k as keyof FormData) : k.startsWith(sec.prefix as string),
  );

  return (
    <div className="space-y-4">
      <h3 className="text-xl font-bold text-center text-blue-600">{sec.title}</h3>
      ...
      <div className="flex justify-between">
        <button
          onClick={() => setIdx((i) => (i - 1 + sections.length) % sections.length)}
